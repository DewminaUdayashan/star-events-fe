...
} from "@/lib/services/admin-hooks";
import { useAllVenues } from "@/lib/services/admin-hooks";
...
const params = useParams();
  ...
  const updateEventMutation = useAdminUpdateEvent();
  
  // Fetch specific event data from /api/admin/events/{eventId}
  const {
    data: eventData,
    isLoading: isLoadingEvent,
    error: eventError,
  } = useAdminEventForEdit(eventId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For admin updates, handle both FormData and JSON based on whether there's a file
      if (formData.imageFile) {
        ...
        // Use JSON for updates without file changes
        const updateData = {
          ...
          prices: formData.priceTiers.map(tier => ({
            category: tier.name,
            price: tier.price,
            stock: tier.availableTickets,
      })),
        };
...
<div className="flex-1">
              <h1 className="text-3xl font-bold text-white">Edit Event (Admin)</h1>
...
export default function AdminEditEventPageWithAuth() {
  return (
