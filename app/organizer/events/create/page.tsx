...
export default function CreateEditEventPage() {
  ...
  // Only load event if we're in edit mode and have an eventId
  const {
    data: existingEvent,
  ...
  }, [isEditMode, existingEvent, isLoadingEvent]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditMode) {
        ...
        // For create, create FormData for multipart/form-data
        const createFormData = new FormData();
        createFormData.append("Title", formData.title);
        createFormData.append("Description", formData.description);
        createFormData.append("EventDate", formData.eventDate);
        createFormData.append("EventTime", formData.eventTime);
        createFormData.append("VenueId", formData.venueId);
        createFormData.append("Category", formData.category);
        createFormData.append("IsPublished", formData.isPublished.toString());
        ...
        if (formData.imageFile) {
          createFormData.append("ImageFile", formData.imageFile);
        }

        await createEventMutation.mutateAsync(createFormData);
      }
              ...
              <CardContent>
                ...
                <p className="text-gray-400 text-sm mt-1">
                  {formData.isPublished
                    ? "Event will be visible to customers"
