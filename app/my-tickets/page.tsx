...
import { Calendar, MapPin, Download, QrCode, Search, Ticket, Clock } from "lucide-react"
...
export default function MyTicketsPage() {
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-6">Please log in as a customer to view your tickets.</p>
          <Link href="/login">
  ...
  const upcomingBookings = filteredBookings.filter((booking) => new Date(booking.event.date) > new Date())

  const pastBookings = filteredBookings.filter((booking) => new Date(booking.event.date) <= new Date())
            ...
            <TabsContent value="upcoming">
              {upcomingBookings.length > 0 ? (
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between mb-2">
                              <h3 className="text-lg font-semibold text-white truncate">{booking.event.title}</h3>
                              <Badge className="bg-green-600 ml-2">{booking.status}</Badge>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="text-sm">
                                ...
                                <p className="text-white font-semibold">Rs. {booking.totalAmount.toLocaleString()}</p>
                              </div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 mb-6">You don't have any tickets for upcoming events.</p>
                  <Link href="/events">
              ...
              {pastBookings.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {pastBookings.map((booking) => (
                    <Card key={booking.id} className="bg-gray-800 border-gray-700 opacity-75">
                      <CardContent className="p-6">
                        <div className="flex gap-4">
                          <div className="relative w-24 h-24 rounded-lg overflow-hidden flex-shrink-0">
                            <Image
