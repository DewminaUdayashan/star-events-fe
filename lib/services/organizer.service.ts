...
import type { Event, OrganizerReport, PaginatedApiResponse } from "../types/api"
...
export class OrganizerService {
  // Events Management
  async getMyEvents(params?: { page?: number; pageSize?: number; status?: string }): Promise<PaginatedApiResponse<Event>> {
    ...
    const url = queryString ? `/api/organizer/events?${queryString}` : "/api/organizer/events"
  ...
  async getEventAnalytics(eventId: string): Promise<EventAnalytics> {
    return apiClient.get<EventAnalytics>(`/api/organizer/events/${eventId}/analytics`)
  }

  async getOrganizerReport(startDate?: string, endDate?: string): Promise<OrganizerReport> {
    const params = new URLSearchParams()
    if (startDate) params.append("startDate", startDate)
    if (endDate) params.append("endDate", endDate)

    const queryString = params.toString()
    const url = queryString ? `/api/Reports/organizer?${queryString}` : "/api/Reports/organizer"
  ...
  async getSalesReport(startDate?: string, endDate?: string): Promise<any> {
    ...
    const queryString = params.toString()
    const url = queryString ? `/api/organizer/sales?${queryString}` : "/api/organizer/sales"
  ...
  // Event Management
  async getEventTickets(eventId: string, params?: { page?: number; pageSize?: number }): Promise<any> {
    const queryParams = new URLSearchParams()
    if (params?.page) queryParams.append('page', params.page.toString())
    if (params?.pageSize) queryParams.append('pageSize', params.pageSize.toString())
    
    const queryString = queryParams.toString()
    const url = queryString ? `/api/organizer/events/${eventId}/tickets?${queryString}` : `/api/organizer/events/${eventId}/tickets`
  ...
  async createPromotion(eventId: string, promotion: any): Promise<any> {
    return apiClient.post(`/api/organizer/events/${eventId}/promotions`, promotion)
  }
  ...
  async updatePromotion(eventId: string, promotionId: string, promotion: any): Promise<any> {
    return apiClient.put(`/api/organizer/events/${eventId}/promotions/${promotionId}`, promotion)
  }

  async deletePromotion(eventId: string, promotionId: string): Promise<void> {
    return apiClient.delete(`/api/organizer/events/${eventId}/promotions/${promotionId}`)
  }
